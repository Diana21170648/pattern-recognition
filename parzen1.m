...
Win_Fc(j)=1/(2*pi)^(1/2)*exp(-1/2*((data2(i,1:4)-data_E1(j,1:4))/h)*((data2(i,1:4)-data_E1(j,1:4))/h)');
end
       p1(i)=1/count(1)/VN*sum(Win_Fc);%求概率密度函数
end
for i=1:size(data2,1)
for j=1:size(data_E2,1)%求训练集E2的窗函数
Win_Fc(j)=1/(2*pi)^(1/2)*exp(-1/2*((data2(i,1:4)-data_E2(j,1:4))/h)*((data2(i,1:4)-data_E2(j,1:4))/h)');
end
       p2(i)=1/count(1)/VN*sum(Win_Fc);
end
for i=1:size(data2,1)
for j=1:size(data_E3,1)%求训练集E3的窗函数
 Win_Fc(j)=1/(2*pi)^(1/2)*exp(-1/2*((data2(i,1:4)-data_E3(j,1:4))/h)*((data2(i,1:4)-data_E3(j,1:4))/h)');
end
        p3(i)=1/count(1)/VN*sum(Win_Fc);
end
%求后验概率
